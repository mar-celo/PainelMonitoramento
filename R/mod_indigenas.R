...
mod_indigenas_server <- function(id) {
  shiny::moduleServer(id, function(input, output, session) {
    ...
    # --------------------------------------------------------------------------
    df_etnia        <- .ler_rds("data-raw/data_indigenas/df_etnia.rds")
    df_efetivos     <- .ler_rds("data-raw/data_indigenas/df_efetivos.rds")
    df_funcao       <- .ler_rds("data-raw/data_indigenas/df_funcao.rds")
    df_funcao_tot   <- .ler_rds("data-raw/data_indigenas/df_funcao_total.rds")
    df_funcao_eqv   <- .ler_rds("data-raw/data_indigenas/df_funcao_efetivos.rds")
    df_natjur       <- .ler_rds("data-raw/data_indigenas/df_natjur.rds")
    df_orgao_raw    <- .ler_rds("data-raw/data_indigenas/df_orgao.rds")
    df_uf           <- .ler_rds("data-raw/data_indigenas/df_indigenas_uf.rds")
    df_pir_ind      <- .ler_rds("data-raw/data_indigenas/df_piramide_indigena.rds")
    df_pir_apf      <- .ler_rds("data-raw/data_indigenas/df_piramide.rds")
    df_tree         <- .ler_rds("data-raw/data_indigenas/df_treemap_ind.rds")
    df_ingressos    <- .ler_rds("data-raw/data_indigenas/serie_ingressos.rds")
    df_cotas        <- .ler_rds("data-raw/data_indigenas/serie_cotas.rds")
    df_mapa         <- .ler_rds("data-raw/data_indigenas/df_mapa_final.rds")
